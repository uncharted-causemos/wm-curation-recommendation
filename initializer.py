...
with app.app_context():
    ontology_file_path = os.path.join(app.root_path, 'data', 'wm_with_flattened_interventions_metadata.yml')
    statements_file_path = os.path.join(app.root_path, 'data', 'statements.json')
...
def init():
    ...
    ontology_spans = convert_concept_examples_and_names_to_embeddings(concepts, examples, nlp)
    ...
    compute_umap_factor_vectors(factor_statement_df, factor_vectors_matrix, mapper)
...
def init_factor_statement_df(statements, nlp, lemmatizer):
    ...
    factor_statement_df = pd.concat([subj_statement_df, obj_statement_df], ignore_index=True)
    factor_statement_df['concept_cat'] = factor_statement_df['concept'].astype('category').cat.codes
...
def convert_concept_examples_and_names_to_embeddings(concepts, examples, nlp):
    ontology_names = list(map(lambda x: x[x.rfind('/') + 1:].replace('_', ' '), concepts))
    flattened_examples = list(map(lambda x: " ".join(x), examples))
    examples_and_names = []
    for i in range(0, len(concepts)):
        examples_and_names.append(ontology_names[i] + " " + flattened_examples[i])
